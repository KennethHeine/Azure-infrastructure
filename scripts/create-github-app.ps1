...
<#
.SYNOPSIS
Creates the shared `claude-runner-agent` GitHub App from github-app/manifest.json
and uploads its private key straight into a target Key Vault.
...
code-driven path is the "app manifest flow", which this script drives
  ...
  4. It prints the two remaining one-tap steps: install the app, and set
     `githubAppId` in the consuming repo's `infra/main.parameters.json`.
...
The app was originally created for `claude-runner` (hence its still-live name
`claude-runner-agent` — the manifest's `name` field is NOT relabeled here, since
editing this file alone would desync it from the actual app without renaming
the live app itself). `claude-runner` was decommissioned 2026-08-05; `agent` is
the platform that now installs and uses it — the default `-ResourceGroup` below
reflects that.

.NOTES
Prereqs: pwsh 7+, a browser on THIS machine (localhost callback), and az CLI
logged in as someone with Key Vault Secrets Officer on the target vault.
Unlike claude-runner's Bicep, agent's has no `adminObjectId` convenience grant
for this — self-grant Secrets Officer on the vault first if you don't already
have it (e.g. via subscription Owner).
#>
param(
  [string]$ManifestPath = (Join-Path $PSScriptRoot '..' 'github-app' 'manifest.json'),
  [string]$ResourceGroup = 'rg-agent',
...
# ---------------------------------------------------------------------------
# 4. Upload the private key to the target repo's Key Vault.
# ---------------------------------------------------------------------------
$vault = az keyvault list -g $ResourceGroup --query '[0].name' -o tsv
if (-not $vault) { throw "No Key Vault found in $ResourceGroup — deploy that repo's infra first." }
...
Write-Host "  2. Set githubAppId = '$($app.id)' in the consuming repo's infra/main.parameters.json (agent's, today) and push to main."
